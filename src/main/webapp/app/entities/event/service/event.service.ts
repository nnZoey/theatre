...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'startTime' | 'endTime' | 'dateBefore'> & {
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...
      dateBefore: event.dateBefore?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...
      dateBefore: restEvent.dateBefore ? dayjs(restEvent.dateBefore) : undefined,
    };
  }
