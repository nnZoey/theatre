...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'issuedDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      issuedDate: order.issuedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      issuedDate: restOrder.issuedDate ? dayjs(restOrder.issuedDate) : undefined,
    };
  }
